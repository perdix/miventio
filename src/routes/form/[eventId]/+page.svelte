<script>
	...
	if (!event.bookingStart) {
		...
		if (event.bookingStart.setHours(0, 0, 0, 0) > new Date().setHours(0, 0, 0, 0)) {
			step = 0;
			warning = {
				title: 'Online-Anmeldung noch nicht geöffnet',
				message: `Die Online-Anmeldung ist ab dem ${event.bookingStart.toLocaleDateString()} möglich!`
			};
		}
		if (
			event.bookingEnd &&
			event.bookingEnd.setHours(0, 0, 0, 0) < new Date().setHours(0, 0, 0, 0)
		) {
			step = 0;
			warning = {
				title: 'Online-Anmeldung geschlossen!',
				message: 'Die Online-Anmeldung ist geschlossen, bitte melden Sie sich vor Ort an!'
			};
		}
	}
	...
	$: total =
		visitor.eventTicket.price +
		visitor.activityTickets.map((a) => a.price).reduce((a, b) => a + b, 0);
	...
	$: {
		if (visitor.categoryId) {
			...
			if (visitor.eventTicket && visitor.eventTicket.dayTicketDate) {
				activityTickets = event.activityTickets
					.filter((a) => a.visitorCategoryId == visitor.categoryId)
					.filter(
						(a) =>
							a.activity.date.toISOString().slice(0, 10) ==
							visitor.eventTicket.dayTicketDate.toISOString().slice(0, 10)
					);
			} else {
			...
			<form class="register row" on:submit|preventDefault={saveAndNext}>
				<div class="col-12">
					...
					<select name="ticket" id="ticket" bind:value={visitor.eventTicket} required>
						{#each tickets as ticket}
							<option value={ticket}
								>{ticket.name} | {ticket.visitorCategory
									? ticket.visitorCategory.name
									: 'Allgemein'} | Preis: {ticket.price}€</option
							>
						{/each}
					</select>
				</div>
				...
				<div class="col-12">
					...
					{#each activityTickets as activityTicket}
						<label
							class="activity"
							class:disabled={activityTicket.activity.limit &&
								activityTicket._count.visitors >= activityTicket.activity.limit}
						>
							<input
								type="checkbox"
								bind:group={visitor.activityTickets}
								value={activityTicket}
								disabled={activityTicket.activity.limit &&
									activityTicket._count.visitors >= activityTicket.activity.limit}
							/>

<script>
	...
	const refreshActivityTickets = async () => {
		...
		fetch(
			`${$page.url.origin}/organisations/${$page.params.organisationId}/events/${$page.params.eventId}/activityTickets`
		)
			.then((r) => r.json())
			.then((tickets) => {
...
<div class="page">
	...
	<Content>
		{#each days as day}
			...
			{#if $event.activities.filter((a) => a.date.substring(0, 10) == day
						.toISOString()
						.substring(0, 10)).length > 0}
				<table>
					<thead>
						<tr>
							<th>ID</th>
					...
					<tbody>
						{#each $event.activities.filter((a) => a.date.substring(0, 10) == day
									.toISOString()
									.substring(0, 10)) as activity}
							<tr on:click={() => editActivity(activity)}>
								...
								<td
									>{activity.start.length > 5 ? activity.start.substring(11, 16) : activity.start} -
									{#if activity.end}

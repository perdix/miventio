<script>
	...
	const deleteTicket = async () => {
		const res = await fetch(`/organisations/${$page.data.organisation.id}/events/${$page.params.eventId}/tickets/${ticket.id}`, {
				method: 'DELETE',
			});
			if (res.status === 200) {
	...
	const saveTicket = async () => {
		if ('id' in ticket) {
			...
			const res = await fetch(`/organisations/${$page.data.organisation.id}/events/${$page.params.eventId}/tickets`, {
				method: 'POST',
				body: JSON.stringify(ticket)
			});
			if (res.status === 201) {

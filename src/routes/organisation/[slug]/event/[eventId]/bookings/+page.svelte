<script>
	...
	let booking = { visits: [{ user: {first_name: ''}, ticket:{}, activities: [], activities_ids: [], activities_prices: []}], phone: '' };
	let showPopup = false;
	let popupTitle = '';
	...
	const newBooking = () => {
		booking = { visits: [{ user: {first_name: ''}, ticket:{}, activities: [], activities_ids: [], activities_prices: []}], phone: '' };
	...
	const addVisit = () => {
		 booking.visits.unshift({ user: {first_name: ''}, ticket:{}, activities: [], activities_ids: [], activities_prices: []});
		 booking = booking;
	}
	...
	const saveBooking = async () => {
		if ('id' in booking) {
			...
			if (res.status === 200) {
				...
				$event.bookings[$event.bookings.findIndex(b => b.id === updatedBooking.id)] = updatedBooking;
				togglePopup();
			}
		} else {
			const res = await fetch(
					...
					<div class="md-12">
						...
						<select id="ticket" bind:value={item.ticket_id} required>
							{#each $page.data.tickets as ticket}
								<option value="{ticket.id}">{ticket.category} - {ticket.name} ({ticket.price}€)</option>	
							{/each}
						</select>
					</div>
					<div class="md-12">
		...
		<tbody>
			{#each $event.bookings as booking}
				<tr on:click={() => editBooking(booking)}>
					...
					<td
						><span class:paid={booking.status.toUpperCase() == 'BEZAHLT'} class:open={booking.status.toUpperCase() == 'OFFEN'}>{booking.status}</span
						></td
					>
				</tr>
			{/each}
